<section class="menu_wrapper migratorios">
    <div class="menu_title content_wrap">
        <h1>Immigration Services</h1>
        <h2>United States</h2>
    </div>
    <div class="menu_items content_wrap">
        <div class="img-item" >
            <a href="<?php echo url_for('guanajuato/index.php?page=immigration/I-130') ?>">
            <img src="<?php echo url_for('private\images\liberty_icon.jpg'); ?>" alt="brandon-mowinkel-UGi_Ng56FRI-unsplash">
            <div>
            <h5>Green Card</h5>
            <p>Procedure to take your relatives to the United States as Legal Permanent Residents</p>
            </div>  
            </a>
        </div>

        <div class="img-item" >
            <a href="<?php echo url_for('guanajuato/index.php?page=immigration/CRBA') ?>">
            <img src="<?php echo url_for('private\images\crba.jpg'); ?>" alt="bonnie-kittle--f7bKsvOgwU-unsplash">
            <div>
            <h5>CRBA</h5>
            <p>Consular Report of Birth Abroad</p>
            </div>  
            </a>
        </div>

        <div class="img-item" >
            <!-- <a href="<?php //echo url_for('guanajuato/index.php?page=translations/index') ?>"> -->
            <img src="<?php echo url_for('private\images/passport.jpg'); ?>" alt="">
            <div>
            <h5>Passport</h5>
            <p>Consular application to obtain a US Passport</p>
            </div>  
            <!-- </a> -->
        </div>
        
        <div class="img-item">
            <img src="<?php echo url_for('private\images/liam-truong-htpU_wGEcW0-unsplash.jpg'); ?>" alt="">
            <div>
            <h5>Certified Documents</h5>
            <p>Request and ammendments of your official documents like Birth, Death, Marriage, and other Certificates.</p>
            </div>
        </div>

        <div class="img-item">
            <img src="<?php echo url_for('private\images/lewis-keegan-XQaqV5qYcXg-unsplash.jpg'); ?>" alt="">
            <div>
            <h5>Apostilles</h5>
            <p>Apostille request for US certified documents.</p>
            </div>
        </div>

        <div class="img-item">
            <img src="<?php echo url_for('private\images/spencer-davis-0QcSnCM0aMc-unsplash.jpg'); ?>" alt="">
            <div>
            <h5>Travel Abroad</h5>
            <p>Travel information to exit and return to Mexico.</p>
            </div>
        </div>
        
        <div class="img-item">
            <img src="<?php echo url_for('private\images/dual.png'); ?>" alt="">
            <div>
            <h5>Citizenship</h5>
            <p>US Citizenship application.</p>
            </div>
        </div>

        <div class="img-item">
            <img src="<?php echo url_for('private\images/giammarco-OPzWvgL-upY-unsplash.jpg'); ?>" alt="">
            <div>
            <h5>Notary Services</h5>
            <p>Notary document preparation for the United States.</p>
            </div>
        </div>

        
    </div>
</section>

